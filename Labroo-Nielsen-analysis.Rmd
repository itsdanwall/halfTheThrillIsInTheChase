...
2. I don't think that WTP is normally distributed (it should be something like a lognormal). 
...
> Half of the participants then learned that they would watch clips from this movie at the end of the experimental session (the approach-sadness condition). The remaining participants moved directly on to the con- ditioning experiment.
> All participants next completed the conditioning experi-
ment, titled “brand evaluation study,” in which they were exposed to one of six sequences (see app. B) of the condi- tioning stimulus materials (conditioning procedures adapted directly from Stuart et al. 1987

> Participants then completed a measure designed to test per-
ceived brand-emotion diagnosticity during the brand-learning phase (modeled after Lee 2001; also see Pleyers et al. 2007). We expected participants who approached sadness to report more positive evaluations of brands conditioned with sad stimuli when emotion was considered diagnostic of the brand (vs. not). Participants responded to each of the following three questions: (1) When viewing different brands during the ex- periment, did you notice systematic links between brand-emo- tion pairings? (2) Was your attention drawn to systematic brand-emotion pairings? (3) Can you state anything about the brand logo presentations (on the computer screen) that stood out?


Not sure how to do this

# Easier is not always better table 1
...
# Easier is not always better table 2
...
of the `r nrep` simulations `r (nlsn_escls_tbl2_reps_sum$psi_sim > nlsn_escls_tbl2_sum$psi_obs) %>% sum` had phis larger than the observed phi

# Easier is not always better table 3
...
```
of the `r reps` simulated `r (nlsn_escls_shpro_tbl3_reps_sum$psi_sim > nlsn_escls_shpro_tbl3_sum$psi_obs) %>% sum` were higher than the observed phi

# Nielsen and Shapiro 2009 Tbl 1
...
of the `r reps` simulations `r (nlsn_shpro_tbl1_reps_sum$psi_sim > nlsn_shpro_tbl1_sum$psi_obs) %>% sum` were higher than the observed phi