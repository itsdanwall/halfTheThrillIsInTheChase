...
This correlation should make the tails of the distribution fatter. 

Some issues with this:

1. Compared to using a non-truncated multivariate normal distribution (Like your simulation), using a truncated normal distribution leads to smaller correlations between the sd of sd's when the mean's is close to the lower/upper bound because many samples hit the bounds. 
...
reps_cors <- replicates_longer %>% 
  group_by(reps, name) %>% 
  summarise(sd_sd = sd(value)) %>% 
  pivot_wider(names_from = name, values_from = sd_sd)
...
```{r}
# replicates_wider <- replicates_longer %>% 
#   pivot_wider(id_cols = reps,
#               values_from = value,
#               names_from = c(manip, name)
#               )
```


```{r}
# check on correlation between items
# cor.test(replicates_wider$ctrl1_app_sen_ind, replicates_wider$ctrl2_app_sen_ind)
...
reps_cors <- replicates %>% 
  group_by(reps) %>% 
  summarise(sd_se_app = sd(app_sen_ind),
            sd_se_prd = sd(prod_eval),
            sd_se_wtp = sd(wtp))
...
cor.test(sd_sd_sim_wide$app_sen_ind, sd_sd_sim_wide$prod_eval)
